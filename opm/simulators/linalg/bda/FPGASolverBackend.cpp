...
FpgaSolverBackend<block_size>::FpgaSolverBackend(std::string fpga_bitstream, int verbosity_, int maxit_, double tolerance_, ILUReorder opencl_ilu_reorder) : BdaSolver<block_size>(fpga_bitstream, verbosity_, maxit_, tolerance_)
{
    ...
    if (verbosity >= 1) {
        ...
        if (perf_call == NULL) {
            OpmLog::warning("Cannot allocate memory for performance counters, disabling performance data collection for FPGA");
    ...
    if (!perf_call_disabled) {
        memset(perf_call, 0, (PERF_RECORDS+1) * sizeof(perf_call_metrics_t));
    }
...
void FpgaSolverBackend<block_size>::get_result(double *x_)
{
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_postprocess = second() - start;
    }
} // end get_result()
...
bool FpgaSolverBackend<block_size>::create_preconditioner()
{
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_preconditioner_create = second() - start;
    }
    return result;
...
void FpgaSolverBackend<block_size>::solve_system(BdaResult &res)
{
    ...
    if (!perf_call_disabled && fpga_calls > PERF_RECORDS) {
        perf_call_disabled = true;
        OpmLog::warning("Too many samples, disabling performance counters for FPGA");
    }
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_mem_setup = second() - start;
    }
    if (err != 0) {
        oss << "Failed to call fpga_copy_to_device_debugbuf (" << err << ")";
        OPM_THROW(std::logic_error, oss.str());
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_mem_h2d = second() - start;
    }
    // ------------------------------------
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_kernel_exec = second() - start;
    }
    if (err != 0) {
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].n_kernel_exec_cycles = kernel_cycles;
    }
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_mem_d2h = second() - start;
    }
...
void FpgaSolverBackend<block_size>::update_system(double *vals, double *b)
{
    ...
    if (!perf_call_disabled) {
        perf_call[fpga_calls].s_reorder = second() - start;
    }
} // end update_system()
...
void FpgaSolverBackend<block_size>::generate_statistics()
{
    ...
    if (perf_call_disabled || perf_call == NULL || fpga_calls == 0) {
    ...
    if (data_points != fpga_calls) {
        OpmLog::warning("FPGA statistics are incomplete because there were too many data points");
        oss.str("");
        oss.clear();
    }
